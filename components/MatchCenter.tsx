    ...
    >
      <div className="flex shrink-0 items-center justify-between border-b border-neutral-100 bg-neutral-50 px-4 py-2">
        <div className="flex min-w-0 items-center">
          <span className="whitespace-nowrap text-xs font-bold uppercase tracking-wider text-neutral-500">{match.league} · 第{match.round}輪</span>
          <span className="ml-2 truncate border-l border-neutral-200 pl-2 text-xs font-medium text-neutral-400">{dateString}</span>
        </div>
        <span className="ml-2 shrink-0 text-xs font-bold text-neutral-400">{isFinished ? '完賽' : timeString}</span>
      </div>
      <div className="flex flex-grow flex-col justify-center p-5">
        <div className="space-y-3">
          {[{ team: homeTeam, score: match.homeScore }, { team: awayTeam, score: match.awayScore }].map(({ team, score }) => (
            <div key={team.id} className="flex items-center justify-between">
              <div className="flex min-w-0 flex-1 items-center space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  <AutoFitText text={team.name} maxFontSize={18} minFontSize={8} className="font-bold tracking-tight text-brand-black" />
                </div>
              </div>
...
const MatchCenter: React.FC = () => {
  ...
  const scheduledMatches = useMemo(
    () => seasonData.matches.filter((match) => match.status === MatchStatus.SCHEDULED).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()),
    [seasonData.matches],
  );
  const finishedMatches = useMemo(
    () => seasonData.matches.filter((match) => match.status === MatchStatus.FINISHED).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()),
    [seasonData.matches],
  );
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="mb-6 flex flex-col items-end justify-between md:flex-row">
            ...
            <div className="mt-4 hidden items-center space-x-2 md:flex">
              <button type="button" onClick={() => scroll('left')} className="rounded-full border border-neutral-200 p-2 text-neutral-500" aria-label="向左捲動"><ChevronLeft className="h-4 w-4" /></button>
              <button type="button" onClick={() => scroll('right')} className="rounded-full border border-neutral-200 p-2 text-neutral-500" aria-label="向右捲動"><ChevronRight className="h-4 w-4" /></button>
              <Link to={`/schedule?season=${activeSeason.id}`} className="ml-4 flex items-center text-sm font-bold text-neutral-500 hover:text-brand-black">完整賽程 <ChevronRight className="ml-1 h-4 w-4" /></Link>
            </div>
          </div>
