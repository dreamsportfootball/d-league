...
const RegistrationOverview: React.FC = () => {
  ...
  const acceptedTeamCount = registrationResults?.acceptedTeams.length ?? 0;
  const waitlistedTeamCount = registrationResults?.waitlistedTeams.length ?? 0;
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:items-start">
          <div className="lg:col-span-5">
            <h2 className="font-display text-4xl font-black uppercase leading-tight tracking-tight text-brand-black md:text-6xl">
              ...
              <span className="block text-brand-blue">{resultsPublished ? '錄取名單' : '賽季報名'}</span>
            </h2>
            <p className="mt-6 max-w-xl text-sm font-medium leading-7 text-neutral-600 md:text-base">
              {resultsPublished
                ? `${acceptedTeamCount} 支球隊正式錄取${waitlistedTeamCount > 0 ? `，${waitlistedTeamCount} 支球隊列入備取` : ''}；L1、L2、L3 正式分組將於完成繳費及參賽確認後公告`
                : `${activeSeason.enabledLeagues.join('、')} ${leagueCountLabel}同步開放報名，正式實施升降級制度`}
            </p>

            {!resultsPublished && <RegistrationProgress variant="compact" />}
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              >
                {resultsPublished ? '查看錄取名單' : '報名詳情'}
                {resultsPublished && <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" />}
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-px overflow-hidden border border-neutral-200 bg-neutral-200 sm:grid-cols-2 lg:col-span-7">
            <div className="bg-white p-6 md:p-8">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-neutral-400">
                {resultsPublished ? '名單公告' : '報名期間'}
              </p>
              <p className="mt-2 font-display text-xl font-bold text-brand-black">
                {resultsPublished
                  ? formatDate(registrationResults?.announcedAt)
            ...
            <div className="bg-white p-6 md:p-8">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-neutral-400">
                {resultsPublished ? '正式錄取' : '預計隊數'}
              </p>
              <p className="mt-2 font-display text-2xl font-black text-brand-black">
                {resultsPublished ? `${acceptedTeamCount} 隊` : `每級別 ${expectedTeamCount ?? 0} 隊`}
              </p>
            </div>
          </div>
        </div>
