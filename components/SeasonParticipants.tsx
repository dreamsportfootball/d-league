  ...
  return (
    <section className={`border-y-2 border-brand-black py-8 md:py-10 ${className}`} aria-labelledby="season-participants-title">
      <div className="flex flex-col gap-4 border-b border-neutral-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-brand-blue">Confirmed Clubs</p>
          <h2 id="season-participants-title" className="mt-2 font-display text-3xl font-black uppercase tracking-tight text-brand-black md:text-5xl">正式參賽隊伍</h2>
          <p className="mt-3 text-xs font-bold tracking-wide text-neutral-500 md:text-sm">正式分級公布於 {formatDate(participants.confirmedAt)}</p>
        </div>
          ...
          return (
            <section key={leagueId} className="border border-neutral-200 bg-white lg:border-0" aria-labelledby={headingId}>
              <div className="flex min-h-20 items-center justify-between bg-brand-black px-5 py-4 text-white">
                <div><p className="text-[10px] font-black uppercase tracking-[0.26em] text-brand-accent">League {leagueId.slice(1)}</p><h3 id={headingId} className="mt-1 font-display text-3xl font-black tracking-tight">{leagueId}</h3></div>
                <span className="text-xs font-black uppercase tracking-widest text-white/65">{teams.length} 隊</span>
              </div>
              <ol className="divide-y divide-neutral-200">
                {teams.map((teamName, index) => {
                  ...
                  return (
                    <li key={teamName}>
                      {publishedTeam ? <Link to={`/teams/${getTeamIdentity(publishedTeam)}?season=${activeSeasonId}`} className="group flex min-h-16 items-center px-5 py-4 transition-colors hover:bg-neutral-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-blue" aria-label={`查看 ${teamName} 球隊頁`} data-scroll-anchor-id={`season-participants:${activeSeasonId}:${leagueId}:${publishedTeam.id}`}>{content}</Link> : <div className="flex min-h-16 items-center px-5 py-4">{content}</div>}
                    </li>
                  );
                })}
              </ol>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2"><p className="flex items-center border-l-2 border-brand-blue pl-4 text-xs font-bold leading-6 text-neutral-600 md:text-sm">{participants.note}</p><p className="border-l-2 border-brand-accent bg-brand-black px-4 py-3 text-xs font-bold leading-6 text-white md:text-sm">{participants.nextStep}</p></div>

      {participants.deadlines.length > 0 && (
        <div className="mt-8 border-t border-neutral-200 pt-8">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-brand-blue">重要期限</p>
          <div className={`mt-4 grid gap-px border border-neutral-200 bg-neutral-200 ${participants.deadlines.length > 1 ? 'md:grid-cols-2' : ''}`}>{participants.deadlines.map((item) => <div key={item.label} className="bg-white p-5 md:p-6"><p className="text-xs font-black tracking-widest text-neutral-500">{item.label}</p><p className="mt-2 font-display text-2xl font-black leading-tight text-brand-black md:text-3xl">{formatDeadline(item.deadline)}</p></div>)}</div>
          <p className="mt-5 border-l-2 border-brand-blue pl-4 text-xs font-bold leading-6 text-neutral-600 md:text-sm">{participants.detailsNote}</p>
        </div>
      )}
    </section>
  );
