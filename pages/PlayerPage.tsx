...
const PlayerPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      <section className="border-b border-neutral-200 bg-neutral-100 px-4 py-8 md:px-12 md:py-10">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-4 overflow-hidden border border-brand-black bg-brand-black text-white">
            <div className="relative grid min-h-[250px] grid-cols-[minmax(0,1fr)_118px] sm:grid-cols-[minmax(0,1fr)_180px] md:min-h-[330px] md:grid-cols-[minmax(0,1fr)_300px]">
              <span
                aria-hidden="true"
                className="pointer-events-none absolute -left-2 bottom-0 font-display text-[9rem] font-black leading-[0.78] text-white/[0.035] sm:text-[12rem] md:text-[17rem]"
              >
                {player.number}
              </span>

              <div className="relative z-10 flex min-w-0 flex-col justify-between p-5 sm:p-6 md:p-8">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-bold uppercase tracking-[0.16em] text-white/55 md:tracking-[0.2em]">
                  <span>D LEAGUE PLAYER</span>
                  <span aria-hidden="true" className="h-1 w-1 rounded-full bg-brand-blue" />
                  <span>{preferredRecord.team?.leagueId ?? 'D LEAGUE'} · {preferredRecord.season.shortName}</span>
                </div>

                <div className="mt-8 min-w-0 md:mt-10">
                  <p className="font-display text-4xl font-black leading-none tracking-tight text-white sm:text-5xl md:text-7xl">
                    {player.name}
                  </p>
                  {player.englishName && (
                    <p className="mt-2 break-words text-[10px] font-semibold uppercase tracking-[0.14em] text-white/45 sm:text-xs md:tracking-wider">
                      {player.englishName}
                    </p>
                  )}
                    ...
                    <div className="min-w-0">
                      {preferredRecord.team && (
                        <Link
                          ...
                          className="block truncate text-sm font-black text-white transition-colors hover:text-brand-blue sm:text-base"
                        >
                          {preferredRecord.team.name}
                        </Link>
                      )}
                      <p className="mt-1 text-[10px] font-semibold text-white/50 sm:text-xs">
                        #{player.number} · {player.nationality} · {player.age} 歲 · {player.gender}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="relative z-10 flex min-h-full items-end justify-center border-l border-white/10 bg-white/[0.04]">
                {image ? (
                  <img
                    src={image}
                    alt={player.name}
                    className="h-full max-h-[250px] w-full object-cover object-top md:max-h-[330px]"
                  />
                ) : (
                  <UserRound className="mb-10 h-14 w-14 text-white/20 md:h-20 md:w-20" />
                )}
              </div>

              <div
                className="absolute inset-x-0 bottom-0 z-20 h-1"
                style={{ backgroundColor: preferredRecord.team?.primaryColor ?? '#0067ff' }}
                aria-hidden="true"
              />
            </div>
          </div>

          <div className="border-x border-b border-neutral-200 bg-white px-4 py-4 sm:px-6 md:px-8 md:py-5">
